...
onMounted(() => {
  ...
  inpComnt.onfocus = () => {  //답글에 포커스가 갔을 때
    if(!cmntClose.classList.contains("userClose")){ //사용자가 답글 대상 정보를 닫았을 경우에는 다시 노출하지 않음
      cmntTo.style.display = "block";
    }
    cmntSubmit.style.display = "block";
  }

  inpComnt.oninput = () => {  //답글 작성 중
    if (inpComnt.scrollHeight > inpComnt.clientHeight) {  //답글 길이에 따라 textarea 길이가 변함
      inpComnt.style.height = inpComnt.scrollHeight + "px";
    } else {
      inpComnt.style.height = (inpComnt.scrollHeight - 20) + "px";
    }

    if (!cmntSubmit.classList.contains("active") && inpComnt.value.length > 0) {  //답글 내용이 있을 경우 등록 버튼 활성화
      cmntSubmit.className += " active";
    }else if (inpComnt.value.length == 0){  //답글 내용이 없을 경우 등록 버튼 비활성화
