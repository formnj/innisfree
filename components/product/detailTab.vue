  ...
  window.addEventListener('scroll', () => {
    ...
    if (
      !props.isMo
        ? (contTop =
            document
              .getElementsByClassName('tab_contents')[0]
        ...
        : (contTop =
            document
              .getElementsByClassName('tab_contents')[0]
              .getBoundingClientRect().y - target.offsetHeight)
    );
...
const tabContShow = (e) => {
  ...
  const tabConts = [
    ...el.closest('ul').parentElement.nextElementSibling.children
  ]
  let contPadding = ''
  !props.isMo
    ? (contPadding =
        document.getElementsByClassName('gnb_wrap')[0].offsetHeight +
        document.getElementsByClassName('tab_wrap')[0].offsetHeight)
    : (contPadding = 60)
  ...
  if (!(tabIdx == 3)) {
    ...
    const contsTop =
      tabConts[tabIdx].getBoundingClientRect().y +
      window.pageYOffset -
      contPadding
